...
import { InternalSubscriptionValue, Subscriber, SubscriberFn, SubscriberIdentifier, SubscriptionValue } from "./types";
...
function registerSubscriber<T, R>(
  ...
  if (depsSet.size !== deps.length) {
    throw new Error("A Subscriber can not depend multiple times on the same subscriber");
  }

  libraryState.subscribers = produce(libraryState.subscribers, subscribers => {
    subscribers.set(id, {
      id: id,
      dependsOn: deps,
      fn: realFn,
...
function getSubscriberDirectChildren(
  ...
  return [
    ...libraryState.subscribers.values(),
...
function getAllSubscribersChildren(
  ...
  return subscribers.reduce<Subscriber<unknown, unknown>[]>((acc, subscriber) => {
...
function getRootSubscribers(): Subscriber<unknown, unknown>[] {
  return [...libraryState.subscribers.values()].filter(s => s.dependsOn.length === 0);
}

function updateSubscribers(produceFn: (subs: Draft<typeof libraryState.subscribers>) => void) {
  libraryState.subscribers = produce(libraryState.subscribers, produceFn);
}

function getSubscriber(id: SubscriberIdentifier): Subscriber<unknown, unknown> | undefined {
...
function computeSubscriberValue(subscriber: Subscriber<unknown, unknown>, ...args: unknown[]): unknown {
...
function subscribe<T>(
  ...
  if (currentComponent !== undefined) {
    ...
    libraryState.subscriberToComponents = produce(libraryState.subscriberToComponents, (subscriberToComponents) => {
